/**************************************
  This class is a subclass of ImageLoopEditor.
  It implements a text-based image loop editor.
  Commands can be read interactively from standard input.
  If a file is named on the command line, editing commands are read from
  that file.

  This editor uses the pushXXX methods defined in ImageLoopEditor.

  There is no need to change this class (but pushXXX methods must be
  defined for this editor to work properly).
...
public class TextImageLoopEditor extends ImageLoopEditor  {
  ...
  public static void textInterface(String[] args) {
    ...
    boolean done = false;
    String [] validCommands = {"save","load","add","insert","find","update",
                                 "edit","jump","display","show","test","remove",
    ...
    while (!done) {
            ...
            if (command.length() > 0 ) {
                ...
                List<String> possibleCommands =
                     selectByPrefix(tokens[0], validCommands);
